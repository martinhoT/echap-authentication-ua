...
Keys are the session tokens themselves and the values are dictionaries like:
{
    ...
    chre_bank: <list of tuples with our challenge and the UAP's response>
...
def echap():
    ...
    if stage == 'start_request':
        ...
        if not error:
            # ATTACKER: create a random password to be used for the Cipher, so that we can create random challenges (which are encrypted)
            password_bytes = secrets.token_bytes(16)
    ...
    chre_bank = echap_session['chre_bank']
    ...
    if stage == 'chre':
        ...
        # Incoming response from UAP to our challenge
            ...
            chre_bank.append((our_challenge, data['chre']))

            # ATTACKER: this is where the attacker's attempt falls apart: it can't correctly answer the UAP's challenge
            uap_challenge_answer = secrets.randbits(128)
            iv = os.urandom(16)
            cipher.mode = modes.CBC(iv)
    ...
    if stage == 'finish':
        ...
        print('Challenge → Response list:')
        for ch,re in chre_bank:
            print(ch,'→',re)
